...
composer.use(filter("balance-button", async (ctx) => {
    try {
        ...
        users = users.filter(
            (user, index) => {
                return allowedUsers[index] && user.status !== "off" && user.status !== "operator"
            }
        );
        ...
        users.forEach((user, userIndex) => {
            ...
            const balanceText = currencies.map(currency => getBalanceText(balances, userIndex, currency, "cash")).join("    ");

            if (!balanceText.trim()) {
                userText.push(`${userNameText} ${ctx.t("user-zero-balance")}`);
            } else {
                userText.push(
